...
function parseCodexFile(content) {
  ...
  for (const line of content.split('\n')) {
    ...
    if ((obj.type === 'turn_context' || p.type === 'turn_context') && typeof p.model === 'string' && p.model) {
    ...
    if (p.type === 'token_count') {
      ...
      if (info && info.total_token_usage) {
        ...
        if (Number.isFinite(ts)) {
          const cur = {
            input: tokenCount(t.input_tokens),
            cached: tokenCount(t.cached_input_tokens),
  ...
  *records() {
  ...
  buildSummary(now = Date.now()) {
    ...
    for (const [key, u] of hourlyModel) {
      ...
      const dateKey = localDateStr(hour);
      const hourWeekKey = localWeekStartStr(hour);
      const hourMonthKey = localMonthStr(hour);
      bump(daily, dateKey, source, tokens, cost);
      bump(weekly, hourWeekKey, source, tokens, cost);
      bump(monthly, hourMonthKey, source, tokens, cost);
      ...
      if (dateKey === todayKey) { kpi.today[source].tokens += tokens; kpi.today[source].cost += cost; }
      if (hourWeekKey === weekKey) { kpi.week[source].tokens += tokens; kpi.week[source].cost += cost; }
      if (hourMonthKey === monthKey) { kpi.month[source].tokens += tokens; kpi.month[source].cost += cost; }
